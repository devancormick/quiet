    ...
    enum ContentMaker {
        static func makeNotificationContent(weeklyMetrics: Int) -> UNMutableNotificationContent {
            if weeklyMetrics > 0 {
                ...
                content.body = NSLocalizedString("Tap to activate Quiet Firewall and update to the newest block lists.", comment: "")
                content.sound = .default
                return content
            }
        }
...
extension PushNotifications {
    ...
    final class EnergySaving {
        ...
        private var requestCounter: Int {
            get {
                return defaults.integer(forKey: kQuietNotificationsEnergySavingCounter)
            }
            set {
                defaults.set(newValue, forKey: kQuietNotificationsEnergySavingCounter)
            }
        }
