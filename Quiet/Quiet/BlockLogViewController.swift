...
class BlockLogViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func configureObservers() {
        kvoObservationToken = defaults.observe(\.QuietDayLogs, options: [.new, .old]) { [weak self] (defaults, change) in
...
fileprivate extension UserDefaults {
    ...
    @objc
    dynamic var QuietDayLogs: [Any]? {
        get {
            return array(forKey: "QuietDayLogs")
        }
        set {
            set(newValue, forKey: "QuietDayLogs")
        }
    }
}
