...
//  QuietiOS
//
//  Copyright © 2018 Quiet, Inc. All rights reserved.
//
...
//        return true
        ...
        setupQuietWhitelistedDomains()
        
        connectivityService.startObservingConnectivity()
        
        // Content Blocker
        SFContentBlockerManager.reloadContentBlocker(withIdentifier: QuietStorageIdentifier.contentBlockerId) { error in
            if error != nil {
        ...
        // Quiet demo: present the minimal calm home screen directly, bypassing
        // the original splash / tab-bar / upsell flow. (Original entry point was
        // `SplashScreenViewController()` — restore that to use the full app.)
        window?.rootViewController = QuietHomeViewController()
        ...
        if (host == "resetsuccessful") {
            ...
            guard let apiCredentials = getAPICredentials() else {
                let popup = PopupDialog(title: "Error",
                                        message: NSLocalizedString("No stored API credentials found. Please contact team@example.com about this error.", comment: ""),
                                        image: nil,
                                        buttonAlignment: .horizontal,
                                        transitionStyle: .bounceDown,
                                        preferredWidth: 270,
            ...
            .done { (signin: SignIn) in
                ...
                setAPICredentialsQuiet(confirmed: true)
                ...
                let popup = PopupDialog(title: "Success! 🎉",
                                        message: NSLocalizedString("Your account has been confirmed and you're now signed in. You'll get the latest block lists, access to Quiet Mac, and get critical announcements.", comment: ""),
                                        image: nil,
                                        buttonAlignment: .horizontal,
                                        transitionStyle: .bounceDown,
                                        preferredWidth: 270,
            ...
            .catch { error in
                ...
                let popup = PopupDialog(title: "Error Confirming Account",
                                        message: "\(NSLocalizedString("Error while trying to confirm your account:", comment: "")) \(errorMessage). \(NSLocalizedString("If this persists, please contact team@example.com.", comment: ""))",
                                        image: nil,
                                        buttonAlignment: .horizontal,
                                        transitionStyle: .bounceDown,
                                        preferredWidth: 270,
