...
//    weak var blockListVC: BlockListViewController?
    ...
    @IBAction func toggleQuiet(sender: UISwitch) {
        setIsQuietEnabled(sender.isOn)
    }
    
    private func setIsQuietEnabled(_ isEnabled: Bool) {
        if let vc = self.blockListVC {
            vc.didMakeChange = true
        }

        lockdownGroup?.enabled = isEnabled
        var ldDefaults = getQuietBlockedDomains()
        ldDefaults.lockdownDefaults[(lockdownGroup?.internalID)!] = lockdownGroup
        
        defaults.set(try? PropertyListEncoder().encode(ldDefaults), forKey: kQuietBlockedDomains)
    }
