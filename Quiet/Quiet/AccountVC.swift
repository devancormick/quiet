...
final class AccountViewController: BaseViewController, Loadable {
    ...
    func createTable() {
        ...
        var message: String? = NSLocalizedString("Sign up below to unlock benefits of a Quiet account.", comment: "")
        ...
        if let apiCredentials = getAPICredentials() {
            ...
            if getAPICredentialsQuiet() == true {
        ...
        let otherCells = [
            ...
            DefaultCell(title: NSLocalizedString("Why Trust Quiet", comment: "")) {
...
func MakeDefaultCell(title: String, color: UIColor = .tunnelsBlue, action: @escaping () -> Void) -> _DefaultCell {
    let cell = _DefaultCell()
    cell.label.text = title
    cell.label.font = .semiboldQuietFont(size: 17)
