...
extension UIView {
    ...
    @discardableResult
    func applyGradient(_ gradient: QuietGradient, corners: Corners = .continuous(0)) -> CAGradientLayer {
    ...
    func showAnimatedSpring(duration: Double = 0.1, withScale scale: CGFloat = 0.95, completion: (() -> Void)? = nil) {
        isUserInteractionEnabled = false
        UIView.animate(withDuration: duration, animations: {
            self.transformToScale(scale)
        }) { (_) in
