...
//            startTutorial()
    ...
    func startTutorial() {
        ...
        let s0 = AwesomeSpotlight(withRect: CGRect(x: centerPoint.x, y: centerPoint.y - 100, width: 0, height: 0), shape: .circle, text: NSLocalizedString("Welcome to the Quiet Tutorial.\n\nTap anywhere to continue.", comment: ""))
        let s1 = AwesomeSpotlight(withRect: getRectForView(firewallTitleLabel).insetBy(dx: -13.0, dy: -13.0), shape: .roundRectangle, text: NSLocalizedString("Quiet Firewall blocks bad and untrusted connections in all your apps - not just Safari.", comment: ""))
    ...
    @IBAction func shareFirewallMetricsTapped(_ sender: Any) {
        ...
        let shareButton = DefaultButton(title: NSLocalizedString("Next", comment: ""), dismissOnTap: true) {
            let shareText = "\(NSLocalizedString("I blocked", comment: "Used in the sentence: I blocked 500 trackers with Quiet.")) \(thousandsFormatter.string(for: getTotalMetrics())!)\(NSLocalizedString(" trackers, ads, and badware with Quiet, the firewall that blocks unwanted connections in all your apps. Get it free at example.com.", comment: "Used in the sentence: I blocked 500 trackers, ads, and badware with Quiet, the firewall that blocks unwanted connections in all your apps. Get it free at example.com."))"
            let vc = UIActivityViewController(activityItems: [QuietCustomActivityItemProvider(text: shareText), image], applicationActivities: [])
                            ...
                            case kApiCodeInvalidAuth, kApiCodeIncorrectLogin:
                                ...
                                confirm.addButtons([
                                   ...
                                   DefaultButton(title: NSLocalizedString("Sign Out", comment: ""), dismissOnTap: true) {
                                    ...
                                    Client.clearCookies()
                                    clearAPICredentials()
                                    setAPICredentialsQuiet(confirmed: false)
...
class QuietCustomActivityItemProvider : UIActivityItemProvider {
