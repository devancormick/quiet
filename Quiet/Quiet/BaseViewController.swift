...
//        self.view.addGestureRecognizer(longPressRecognizer)
    ...
    func popupErrorAsNSURLError(_ error: Error) -> Bool {
        ...
        if nsError.domain == NSURLErrorDomain {
            self.showPopupDialog(title: NSLocalizedString("Network Error", comment: ""), message: NSLocalizedString("Please check your internet connection. If this persists, please contact team@example.com.\n\nError Description\n", comment: "") + nsError.localizedDescription, acceptButton: NSLocalizedString("Okay", comment: ""))
            return true
        }
        else {
            return false
    ...
    func showWhyTrustPopup() {
        let popup = PopupDialog(
            title: NSLocalizedString("Why Trust Quiet?", comment: ""),
            message: NSLocalizedString("Quiet is open source and fully transparent, which means anyone can see exactly what it's doing. Also, Quiet Firewall has a simple, strict Privacy Policy, while Quiet VPN is fully audited by security experts.", comment: ""),
    ...
    func showVPNDetails() {
        self.showModalWebView(title: NSLocalizedString("Secure Tunnel VPN", comment: ""), urlString: "https://example.com/secure-tunnel")
//        let popup = PopupDialog(
//            title: NSLocalizedString("About Quiet VPN", comment: ""),
//            message: NSLocalizedString("Quiet VPN is powered by Quiet VPN, the open source, no-logs, and fully audited VPN.", comment: ""),
//            buttonAlignment: .vertical,
//            transitionStyle: .bounceDown,
//            preferredWidth: 300.0,
//            tapGestureDismissal: true,
...
//            self.showModalWebView(title: NSLocalizedString("Why Use VPN?", comment: ""), urlString: "https://example.com/why-vpn")
//        }
//
//        let auditReportsButton = DefaultButton(title: NSLocalizedString("Audit Reports", comment: ""), dismissOnTap: true) {
//            self.showAuditModal()
//        }
//
//        let confirmedWebsiteButton = DefaultButton(title: NSLocalizedString("Quiet Site", comment: ""), dismissOnTap: true) {
//            self.showModalWebView(title: NSLocalizedString("Why Use VPN?", comment: ""), urlString: "https://example.com")
//        }
...
//        self.present(popup, animated: true, completion: nil)
    ...
    func handlePurchaseSuccessful(placement: PurchasePlacement = .homeScreen, completion: (()->Void)? = nil) {
        ...
        if let apiCredentials = getAPICredentials(), getAPICredentialsQuiet() == true {
    ...
    func handlePurchaseFailed(error: Error) {
        if let skError = error as? SKError {
            ...
            switch skError.code {
            case .unknown:
                errorText = .localized("Unknown error. Please contact support at team@example.com.")
            case .clientInvalid:
...
""")
            ...
            self.showPopupDialog(
                ...
                message: .localized("Please contact team@example.com.\n\nError details:\n") + "\(error)",
    ...
    func showPrivacyPolicyModal() {
        self.showModalWebView(title: NSLocalizedString("Privacy Policy", comment: ""), urlString: "https://example.com/privacy")
    }
    
    func showTermsModal() {
        self.showModalWebView(title: NSLocalizedString("Terms", comment: ""), urlString: "https://example.com/terms")
    }
    
    func showFAQsModal() {
        self.showModalWebView(title: NSLocalizedString("FAQs", comment: ""), urlString: "https://example.com/faq")
    }
    
    func showWebsiteModal() {
        self.showModalWebView(title: NSLocalizedString("Website", comment: ""), urlString: "https://example.com")
    }
    
    func showWebsitePressModal() {
        self.showModalWebView(title: NSLocalizedString("Press & Media", comment: ""), urlString: "https://example.com/#press")
    }
    
    func showAuditModal() {
        self.showModalWebView(title: NSLocalizedString("Audit Reports", comment: ""), urlString: "https://openaudit.com/example")
    }
        ...
        VPNController.shared.isConfigurationExisting { (exists) in
            if exists {
                ...
                self.showPopupDialog(
                    ...
                    message: "Due to a recent iOS or Quiet update, the Firewall needs to be refreshed to run properly.\n\nIf asked, tap \"Allow\" on the next dialog to automatically complete this process.",
                    buttons: [
                        .cancel(),
...
//            self.emailTeam(messageBody: "Hey Quiet Team, \nI encountered a bug while using Quiet, and I'm reporting it here. \n (To the user: just tap Send at the top right to submit the bug report -- no need to do anything else and we'll get back to you ASAP.", messageErrorBody: error ? error! || "")
//        }
//        popup.addButtons([acceptButton, submitButton])
//        self.present(popup, animated: true, completion: nil)
//    }
    
    @objc func emailTeam(messageBody: String = NSLocalizedString("Hi, my question or feedback for Quiet is: ", comment: ""), messageErrorBody: String = "") {
        DDLogInfo("")
        ...
        let subject = "Quiet Question or Feedback (iOS \(Bundle.main.versionString))" + appendString
    ...
    func sendMessage(_ message: String, subject: String) {
        let recipient = "team@example.com" // TODO: change email
