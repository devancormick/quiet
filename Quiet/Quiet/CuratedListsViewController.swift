...
final class CuratedListsViewController: UIViewController {
    ...
    var lockdownBlockLists: [QuietGroup] = []
    var basicQuietBlockLists: [QuietGroup] = []
    var advancedQuietBlockLists: [QuietGroup] = []
    ...
    func reloadTableView() {
        ...
        lockdownBlockLists = {
            let domains = getQuietBlockedDomains().lockdownDefaults
            let sorted = domains.sorted(by: { $0.key < $1.key })
            return Array(sorted.map(\.value))
        }()
        
        basicQuietBlockLists = lockdownBlockLists.filter{ $0.accessLevel == "basic"}
        advancedQuietBlockLists = lockdownBlockLists.filter{ $0.accessLevel == "advanced"}
...
extension CuratedListsViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        let numberOfBasicLists = basicQuietBlockLists.count
        let numberOfAdvancedLists = advancedQuietBlockLists.count
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            blockListView.contents = .lockdownGroup(basicQuietBlockLists[indexPath.row])
        ...
        case 1:
            let cell = UITableViewCell()
            let blockListView = BlockListView()
            blockListView.contents = .lockdownGroup(advancedQuietBlockLists[indexPath.row])
            cell.contentView.addSubview(blockListView)
            blockListView.anchors.edges.pin()
            cell.accessoryType = .disclosureIndicator
            return cell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 0:
            ...
            target.lockdownGroup = basicQuietBlockLists[indexPath.row]
        ...
        case 1:
            if UserDefaults.hasSeenAdvancedPaywall || UserDefaults.hasSeenAnonymousPaywall || UserDefaults.hasSeenUniversalPaywall {
                let storyboard = UIStoryboard.main
                let target = storyboard.instantiate(BlockListGroupViewController.self)
                target.lockdownGroup = advancedQuietBlockLists[indexPath.row]
                target.blockListVC = self
                self.navigationController?.pushViewController(target, animated: true)
            } else {
