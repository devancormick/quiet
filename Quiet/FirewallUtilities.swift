...
struct QuietGroup : Codable {
    ...
    var ipRanges : Dictionary<String, QuietIPRange>
    var warning: String?
    var accessLevel: String = "advanced"
}

struct QuietDefaults : Codable {
    var lockdownDefaults : Dictionary<String, QuietGroup>
}
...
func setupFirewallDefaultBlockLists() {
    var lockdownBlockedDomains = getQuietBlockedDomains()
    
    let snapchatAnalytics = QuietGroup.init(
        version: 28,
    ...
    let emailOpens = QuietGroup.init(
        version: 31,
    ...
    let facebookInc = QuietGroup.init(
        version: 34,
    ...
    let facebookSDK = QuietGroup.init(
        version: 29,
    ...
    let marketingScripts = QuietGroup.init(
        version: 32,
    ...
    let marketingScriptsII = QuietGroup.init(
        version: 31,
    ...
    let googleShoppingAds = QuietGroup.init(
        version: 36,
    ...
    let dataTrackers = QuietGroup.init(
        version: 36,
    ...
    let generalAds = QuietGroup.init(
        version: 41,
    ...
    let amazonTrackers = QuietGroup.init(
        version: 33,
    ...
    let ifunnyTrackers = QuietGroup.init(
        version: 4,
    ...
    let advancedGaming = QuietGroup.init(
        version: 5,
    ...
    let tiktokTrackers = QuietGroup.init(
        version: 4,
    ...
    let junesJourneyTrackers = QuietGroup.init(
        version: 5,
    ...
    let advancedAnalytics = QuietGroup.init(
        version: 4,
    ...
    for var defaultGroup in defaultQuietSettings {
    ...
    defaults.set(try? PropertyListEncoder().encode(lockdownBlockedDomains), forKey: kQuietBlockedDomains)
}
...
func getAllBlockedDomains() -> Array<String> {
    let lockdownBlockedDomains = getQuietBlockedDomains()
...
// MARK: - Curated Quiet blocked domains

func getQuietBlockedDomains() -> QuietDefaults {
    guard let lockdownDefaultsData = defaults.object(forKey: kQuietBlockedDomains) as? Data else {
        return QuietDefaults(lockdownDefaults: [:])
    }
    guard let lockdownDefaults = try? PropertyListDecoder().decode(QuietDefaults.self, from: lockdownDefaultsData) else {
        return QuietDefaults(lockdownDefaults: [:])
    }
